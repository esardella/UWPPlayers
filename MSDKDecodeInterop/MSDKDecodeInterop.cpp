...
MSDKInterop ^ MSDKDecodeInterop::MSDKInterop::CreatefromFile(Windows::Storage::StorageFile^ file)
{
	...
	if (FAILED(msdkDecoder->pipeline.SetFileSource(file)))
	{
		msdkDecoder = nullptr; 
	}
	else
...
MSDKInterop ^ MSDKDecodeInterop::MSDKInterop::CreatefromURI(String ^ uri)
{
	auto msdkDecoder = ref new MSDKInterop();
	if (FAILED(msdkDecoder->pipeline.SetURISource(uri))) 
	{
		msdkDecoder = nullptr; 
	}
	else
